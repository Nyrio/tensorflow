...
TRTEngineOp::TRTEngineOp(OpKernelConstruction* context)
  ...
  OP_REQUIRES_OK(context,
                 context->GetAttr("InT", &in_types));
  for(int i=0; i < input_mask_.size(); i++) {
    input_mask_[i] = (in_types[i] != DataType::DT_RESOURCE);
  }
  /// TODO: this makes the input_mask op attribute unnecessary -> cleanup.
  ...
  VLOG(2) << "input_partial_shapes_.size()" << input_partial_shapes_.size();
  VLOG(2) << "input_partial_shapes_filtered_.size()" << input_partial_shapes_filtered_.size();
...
Status TRTEngineOp::VerifyInputShapes(
  ...
  if (input_partial_shapes_filtered_.empty()) {
    ...
    for (int i = 0; i < input_concrete_shapes.size(); i++) {
      if (input_concrete_shapes[i].dims() != input_partial_shapes_filtered_[i].dims()) {
        return errors::InvalidArgument(error_msg);
      }
    }
    for (int i = 0; i < input_concrete_shapes.size(); i++) {
...
void TRTEngineOp::ComputeAsync(OpKernelContext* ctx,
  ...
  Status verify_input_shape_status = VerifyInputShapes(input_concrete_shapes_filtered);
