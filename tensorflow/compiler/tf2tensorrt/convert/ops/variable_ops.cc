...
class ConvertVariableV2 : public OpConverterBase<ConvertVariableV2> {
 public:
  ...
  static constexpr std::array<InputArgSpec, 0> InputSpec() {
    return {};
  }
  ...
  Status ValidateAux() {
    ...
    StatusOr<TensorShapeProto> shape_proto = GetAttrValue<TensorShapeProto>("shape");
  ...
  Status Validate() {
    ...
    switch (attrs_.dtype) {
      case DT_FLOAT:
        return ValidateAux<float>();
      case DT_HALF:
        return ValidateAux<Eigen::half>();
    }
  }
  ...
  Status Convert() {
    switch (attrs_.dtype) {
      case DT_FLOAT:
        return ConvertAux<float>();
      case DT_HALF:
        return ConvertAux<Eigen::half>();
    }
  }
